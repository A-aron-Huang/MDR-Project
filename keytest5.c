...
void servoWrite(int angle){    //Specif a certain rotation angle (0-180) for the servo
    if(angle > 180)
        angle = 180;
    if(angle < 0)
        angle = 0;
...
int main(){
	...
	printw("Key commands:\n");
	printw("WSAD - Move Foward, Back, Left, and Right\n");
	printw("QE - Move Forward-Left and Forward-Right\n");
